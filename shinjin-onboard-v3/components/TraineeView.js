    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #f0f7ff 0%, #e0efff 100%)', fontFamily: 'sans-serif', paddingBottom: '80px' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e40af, #2563eb)', padding: '16px 20px', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '36px', height: '36px', background: 'white', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' }}>{isDebut ? 'â­' : 'ðŸŽ“'}</div>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'white', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-around', padding: '10px 0 20px' }}>
        {[{ id: 'home', icon: 'ðŸ ', label: 'ãƒ›ãƒ¼ãƒ ' }, { id: 'training', icon: 'ðŸ“š', label: 'ç ”ä¿®' }, { id: 'shift', icon: 'ðŸ“…', label: 'ã‚·ãƒ•ãƒˆ' }].map(item => (
  ...
  <>
    ...
    <div style={{ background: 'white', borderRadius: '16px', padding: '20px', marginBottom: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <span style={{ fontWeight: '600', color: '#1e293b' }}>ðŸ“š ç ”ä¿®é€²æ—</span>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {curriculum.filter(c => c.done).length === curriculum.length ? <span style={{ fontSize: '14px', color: '#16a34a', fontWeight: '600' }}>ðŸŽ‰ å®Œäº†ï¼</span> : <span style={{ fontSize: '14px', color: '#64748b' }}>æ®‹ã‚Š {curriculum.filter(c => !c.done).length} é …ç›®</span>}
      </div>
    </div>

    {/* é …ç›®åˆ¥ã‚¹ãƒ†ãƒ¼ã‚¿ã‚¹ */}
    <div style={{ background: 'white', borderRadius: '16px', padding: '20px', marginBottom: '16px' }}>
      <div style={{ fontWeight: '600', color: '#1e293b', marginBottom: '16px' }}>ðŸ“Š é …ç›®åˆ¥ã‚¹ãƒ†ãƒ¼ã‚¿ã‚¹</div>
